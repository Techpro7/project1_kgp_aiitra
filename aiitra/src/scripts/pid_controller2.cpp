...
#define printVector4(x) for(int i = 0 ; i < 4; i++){std::cout<<x(i)<<" ";} std::cout<<"\n";
#define printVectorN(x, n) for(int i = 0 ; i < n; i++){std::cout<<x(i)<<" ";} std::cout<<"\n";
...
Eigen::Vector4d PID( State des_state, State curr_state){
	...
	std::cout<<"err_angle = ";printVector3(err_angle);
	...
	std::cout<<"ang_dd = ";printVector3(ang_dd);
...
int main(int argc, char **argv){
	...
	std::cout<<"INITIALIZING PARAMETERS OF DRONE AND ENVIRONMENT...\n";
	initialise_params(n, mav, abs_gravity_z);
	abs_gravity_z = abs_gravity_z < 0 ? -1*abs_gravity_z : abs_gravity_z;
	std::cout<<"...DONE\n";

	std::cout<<"INITIALIZING ROTOR MATRIX...\n";
	InitialiseRotorMatrix();
	std::cout<<"Rotor Matrix :\n";
	PRINT_MAT(mav.inp_to_rotor_matrix);
	std::cout<<"...DONE\n";
	...
	while(ros::ok()){
		...
		std::cout<<"Control Input = ";printVector4(input_U);
		
		/*for(int i = 0; i < 4; i++){
			if( std::isnan( input_U(i,0) ) ){
				input_U(i,0) = 0;
			}
		...
		//std::cout<<"Calculated Rotor Speeds = ";printVectorN(rs, mav.parameters.no_rotors);
