...
class GotoPoint():
    def __init__(self):
        rospy.init_node('aiitra_robot_key', anonymous=False) #aiitra_robot_key
        rospy.on_shutdown(self.shutdown)
        self.cmd_vel_robot = rospy.Publisher('cmd_vel_robot', Twist, queue_size=5) #changes for our bot
        position = Point()
        move_cmd = Twist()
        r = rospy.Rate(10)
        ...
        try:
            self.tf_listener.waitForTransform(self.odom_frame, 'origin_link', rospy.Time(), rospy.Duration(1.0))
            self.base_frame = 'origin_link'
        ...
        while distance > 0.05:
            ...
            self.cmd_vel_robot.publish(move_cmd) #cmd_vel_robot
            r.sleep()
        ...
        self.cmd_vel_robot.publish(Twist()) #cmd_vel_robot
        return
